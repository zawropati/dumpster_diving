<template>
<div class="flex" style="height:100%">
    <div class="sign-box">
        <div class="signin">
            <h2>Log in</h2>
            <input class="input" type="text" id="email" v-model="email" placeholder="Email">
            <input class="input" type="password" id="userPassword" v-model="userPassword" placeholder="Password">
            <div>
                <v-btn class="action-btn" @click="signInUser()">Log in</v-btn>
            </div>
            <div>
                <v-btn class="action-btn2" @click="signUp()">Don't have an account yet? Sign up!</v-btn>
            </div>
        </div>
    </div>
    <div class="singin-image">
        <div class="flex">
            <svg enable-background="new 0 0 512 512" height="150" viewBox="0 0 512 512" width="70" xmlns="http://www.w3.org/2000/svg"><g><path d="m315.959 30.463c-13.682-18.378-35.38-30.463-60-30.463h-15v30h15c24.814 0 45 20.186 45 45h15 15c0-16.732-5.701-32.045-15-44.537z" fill="#575f64"/><path d="m315.959 30.463v44.537h15c0-16.732-5.701-32.045-15-44.537z" fill="#474f54"/><path d="m430.495 245.97c-13.403-21.313-13.154-46.641-10.605-64.16 1.948-13.359 1.23-26.982-2.109-40.532-10.514-42.675-49.448-80.975-101.821-80.818-60.853.189-105 50.629-105 106.54 0 12.376 2.018 16.463 2.271 30.923l9.976 222.261c4.503 2.254 39.752 31.906 92.754 31.954.038 0 .079.009.117.009 72.748 0 134.883-59.15 134.883-135.146-.002-25.182-7.078-49.732-20.466-71.031z" fill="#fed843"/><path d="m450.959 317c0-25.181-7.075-49.731-20.464-71.03-13.403-21.313-13.154-46.641-10.605-64.16 1.948-13.359 1.23-26.982-2.109-40.532-10.515-42.674-49.447-80.974-101.821-80.818v391.678c.038 0 .079.009.117.009 72.747-.001 134.882-59.15 134.882-135.147z" fill="#fabe2c"/><path d="m165.959 162.618c-2.871-4.338-5.938-8.58-9.595-12.449-16.934-17.9-40.767-28.169-65.405-28.169v30c16.67 0 32.153 6.665 43.608 18.779 11.44 12.114 17.241 27.993 16.304 44.722l15.088.842 14.854.828c1.105-19.702-4.243-38.524-14.854-54.553z" fill="#575f64"/><path d="m165.959 162.618v53.725l14.854.828c1.105-19.702-4.243-38.524-14.854-54.553z" fill="#474f54"/><path d="m248.341 183.274c-20.654-3.706-41.851.63-61.348 12.319-6.304 3.759-13.627 5.698-21.035 5.709-7.205.011-14.495-1.798-21.006-5.68-19.541-11.733-40.767-16.069-61.377-12.349-56.997 10.049-93.871 77.871-80.951 151.187 10.276 58.289 54.625 116.002 113.149 116.002 17.999 0 35.317-5.55 50.186-16.523 16.216 11.221 33.354 16.714 51.885 16.479 52.354-1.172 98.804-49.937 110.449-115.957 12.919-73.315-22.955-141.138-79.952-151.187z" fill="#ff641a"/><path d="m328.293 334.461c12.92-73.315-22.954-141.138-79.951-151.187-20.654-3.706-41.851.63-61.348 12.319-6.304 3.759-13.627 5.698-21.035 5.709v232.635c16.216 11.221 33.354 16.714 51.885 16.479 52.353-1.17 98.803-49.935 110.449-115.955z" fill="#f03800"/><path d="m472.169 425 34.395-34.395-21.211-21.21-60 60v10.605 10.605l60 60 21.211-21.21-34.395-34.395h39.79v-15-15z" fill="#65d6a9"/><path d="m485.353 510.605 21.211-21.21-34.395-34.395h39.79v-15h-86.606v10.605z" fill="#26bfa6"/><path d="m398.019 368.721-7.061.707-15 25.573-15-22.575-90 8.998-15 28.577-15-25.578-76.641 7.663c-24.727 2.476-43.359 23.071-43.359 47.915s18.633 45.439 43.359 47.915c58.141 5.792 102.653 10.247 136.641 13.651l15-31.566 15 34.57c52.335 5.239 69.595 7.43 71.821 7.43 26.686 0 48.179-22.129 48.179-48.636v-23.365-23.364c.001-28.517-24.717-50.796-52.939-47.915z" fill="#ff9100"/><path d="m164.318 487.915c58.141 5.792 102.653 10.247 136.641 13.651l15-31.566 15 34.57c52.335 5.239 69.595 7.43 71.821 7.43 26.686 0 48.179-22.129 48.179-48.636v-23.364h-330c0 24.844 18.632 45.439 43.359 47.915z" fill="#ff641a"/><g><path d="m300.959 455v46.566c11.184 1.121 21.01 2.104 30 3.005v-49.571z" fill="#ff9100"/><g fill="#fabe2c"><path d="m270.959 425v-43.577l-30 2.999v40.578z"/><path d="m390.959 425v-55.573l-30 2.998v52.575z"/></g></g></g></svg>
            <svg id="Layer_1" enable-background="new 0 0 512 512" height="150" viewBox="0 0 512 512" width="70" xmlns="http://www.w3.org/2000/svg"><g><path d="m471.625 180.59c37.376-71.014-54.696-138.844-111.441-82.1l-62.694 62.693c-37.519 37.519-91.672 63.771-153.973 63.771-78.655 0-142.735 63.601-143.51 142.093-.804 81.419 67.109 146.52 148.517 144.924 149.837-2.937 213.296-122.75 220.229-135.921z" fill="#78116e"/><path d="m148.524 511.971c149.837-2.937 213.297-122.75 220.229-135.921l102.873-195.46c16.255-30.885 8.02-61.162-11.14-80.856l-391.011 391.011c23.012 13.963 50.18 21.792 79.049 21.226z" fill="#50003c"/><path d="m454.173 105.94c-5.893-5.893-5.855-15.46.094-21.307 15.273-15.314 27.646-37.301 27.646-69.633 0-8.284 6.716-15 15-15s15 6.716 15 15c0 1.632 3.661 50.752-36.525 90.938-5.803 5.805-15.304 5.913-21.215.002z" fill="#00b464"/><path d="m454.26 218.426-27.739-27.739h-41.987c-8.284 0-15-6.716-15-15v-41.987l-27.739-27.74c-3.733-3.733-5.229-9.154-3.938-14.273s5.178-9.184 10.235-10.7l46.375-13.913c27.967-8.389 58.165-.776 78.811 19.87 20.647 20.646 28.262 50.845 19.87 78.812l-13.913 46.376c-1.518 5.057-5.581 8.943-10.7 10.234-.195.047-7.997 2.339-14.275-3.94z" fill="#82fa73"/><path d="m384.533 190.687h41.987l27.739 27.739c6.278 6.278 14.081 3.987 14.273 3.938 5.119-1.291 9.183-5.178 10.7-10.234l13.913-46.376c8.392-27.967.777-58.165-19.87-78.812l-99.35 99.35c2.716 2.716 6.466 4.395 10.608 4.395z" fill="#41d76c"/></g></svg>
            <svg id="Layer_2" enable-background="new 0 0 512 512" height="150" viewBox="0 0 512 512" width="70" xmlns="http://www.w3.org/2000/svg"><path id="XMLID_18_" d="m420.5 265.8-.3.3c-11.1 13.8-20.7 27-29.2 39.5l.1.1c10.9 6.6 25 7 36.5-.4 8.8-5.6 14.2-14.5 15.6-24l.1-.3c-9-2.3-17-7.9-22.8-15.2z" fill="#3b601c"/><path id="XMLID_53_" d="m325.8 244.4-23.4 50.3c-18.2 8.9-41.8-1.5-49.8-13.3l-3.6-20.3c5.3.8 10.7 1.2 16.2 1.2 21.9.1 42.6-6.4 60.6-17.9z" fill="#4e7a25"/><path id="XMLID_54_" d="m180.4 222.5c7.8 8.5 16.3 15.9 25.4 21.9-4.5 30-42.7 45.2-66.1 25.4-5.3-10.3-8.9-17.4-9.1-17.8 12.5-.3 25-4.6 35.6-13.1 5.9-4.8 10.7-10.3 14.2-16.4z" fill="#3b601c"/><path id="XMLID_55_" d="m81.7 251.7 32.8 63.1-.4.2c-21.6 10-47.2.5-57.2-21.1-6.2-13.5-4.8-28.6 2.3-40.4l.7-2.8c1.3.3 2.7.6 4.1.9 6 1 12 1 17.7.1z" fill="#3b601c"/><g id="XMLID_1_"><path id="XMLID_11_" d="m66.9 283.9c-4.8-10.5-5.1-22-1.6-32.1-.5-.1-.9-.1-1.4-.2-1.4-.2-2.7-.5-4.1-.9l-.7 2.8c-7.2 11.8-8.5 26.9-2.3 40.4 9.9 21.6 35.5 31.1 57.2 21.1l.5-.2-3.2-6.1c-18.1 2.2-36.3-7.3-44.4-24.8z" fill="#4d841e"/></g><g id="XMLID_12_"><path id="XMLID_13_" d="m190.9 232.8c-6.5 23.7-34.7 36.4-56.5 26.4 1.5 2.9 3.3 6.4 5.4 10.5 23.3 19.8 61.5 4.7 66.1-25.4-5.2-3.3-10.2-7.2-15-11.5z" fill="#315112"/></g><path id="XMLID_15_" d="m249 261.5 3.5 19.9c7.9 11.8 31.6 22.2 49.8 13.3l8.9-19.2c-22.6 7.6-50.3-2-62.2-14z" fill="#2c4415"/><g id="XMLID_16_"><path id="XMLID_17_" d="m427.6 291.3c-9.1 5.8-19.8 6.8-29.3 3.7-2.5 3.6-5 7.1-7.3 10.6l.1.1c10.9 6.6 25 7 36.5-.4 8.8-5.6 14.2-14.5 15.6-24l.1-.3c-1.5-.4-3-.9-4.4-1.4-2.7 4.6-6.5 8.7-11.3 11.7z" fill="#4d841e"/></g><path d="m331.8 230.4-64.1 136.8-18.7-106.1c-15.4-2.4-30.1-8.1-43.1-16.8l8.1 97.5c.1 22.3-21 28.5-31.5 9.7l-59.5-114.7-41.8 14 113.1 218c6.4 16.3 22.1 27.1 39.8 27.1h46.1c18.7 0 34.7-12.4 40.8-30.1 16.5-47.6 30-96 72-158.3l36.3-52.6z" fill="#cadd78"/><g fill="#adbc55"><path d="m296.7 488.4h-46.1c-17.6 0-33.4-10.8-39.8-27.1l-111.7-215.6-17.4 5.9 112.6 217.2c6.4 16.3 22.1 27.1 39.8 27.1h46.1c9.1 0 18-3 25.3-8.5-2.9.7-5.8 1-8.8 1z"/><path d="m223.9 254c-6.3-2.7-12.3-5.9-18-9.6l8.1 97.5c.1 4.4-.9 8.8-2.9 12.7 9.6 2.7 19.5-5 19.5-20.3z"/><path d="m333.8 224.5-66 142.9 2.9 16.3c1.2 9.4 5.1 11.2 9.5 9.8 3.7-1.1 6.5-4.1 7.9-7.7l70.6-161.2z"/></g><path id="XMLID_27_" d="m88.6 156c-19.3 20.8-21.4 52.7-3.5 75 4.6 5.7 10 10.2 16 13.6-10.8 6.4-23.8 9.2-37.1 6.9-29.7-5-49.7-33.2-44.7-62.9s33.2-49.7 62.9-44.7l1.4-.4c1.2 4.4 2.8 8.5 5 12.5z" fill="#3b601c"/><path id="XMLID_26_" d="m415 253.6c-13 5.3-27.3 6-40.6 2.3-10.9-3-21.1-9.1-29.3-18-1.5-1.6-2.8-3.2-4.1-4.9l-.2.1c-21.1 18.4-47.3 29.3-75.6 29.3-33.4 0-62.1-15.1-84.7-39.9-3.6 6.1-8.3 11.6-14.1 16.3-19.3 15.5-45.3 17.1-65.2 5.8-6-3.4-11.4-7.9-16-13.6-17.8-22.2-15.8-54.1 3.5-75-2.2-4-3.8-8.1-5-12.4-11.8-44.2 31.8-99.3 90.1-67.6 32.9-88.1 154.6-69.1 181.8-7.6 41.9-20.6 86.8 21.8 71.1 54.9h1.5c34.8 0 63 28.2 63 62.9 0 11.4-3.1 22.2-8.4 31.4 6.6 7 10.7 16.6 10.7 27 0 21.8-17.7 39.5-39.5 39.5-18 0-34.2-14.2-39-30.5z" fill="#4d841e"/><g id="XMLID_2_"><path id="XMLID_10_" d="m453.9 244.1c-17.9 0-34.1-14.2-39-30.6-13 5.3-27.3 6-40.6 2.3-10.9-3-21.1-9-29.3-18-1.5-1.6-2.8-3.2-4.1-4.9l-.2.1c-21.1 18.4-47.3 29.3-75.6 29.3-33.4 0-62.1-15.1-84.7-39.9-3.6 6.1-8.3 11.6-14.1 16.3-19.3 15.5-45.3 17.1-65.2 5.8-6-3.4-11.5-7.9-16-13.6-3.8-4.7-6.6-9.8-8.7-15.2-6.7 18.3-4.2 39.2 8.7 55.2 4.6 5.7 10 10.2 16 13.6 19.9 11.2 45.9 9.7 65.2-5.8 5.8-4.7 10.6-10.2 14.1-16.3 22.6 24.7 51.3 39.9 84.7 39.9 28.3 0 54.5-10.9 75.6-29.3l.2-.1c1.3 1.7 2.6 3.3 4.1 4.9 8.2 8.9 18.4 14.9 29.3 18 13.3 3.7 27.6 3 40.6-2.3 4.8 16.4 21 30.6 39 30.6 21.8 0 39.5-17.7 39.5-39.5 0-7.3-2-14.1-5.4-20-6.9 11.7-19.5 19.5-34.1 19.5z" fill="#67a829"/></g><g id="XMLID_6_"><path id="XMLID_7_" d="m85.8 231.8c-.6-.1-1.2-.2-1.8-.3-29.7-5-49.7-33.2-44.7-62.9 1.1-6.6 3.4-12.7 6.5-18.1-13.6 7.8-23.7 21.5-26.5 38.1-5 29.7 15 57.9 44.7 62.9 13.3 2.2 26.3-.5 37.1-6.9-5.7-3.2-10.9-7.5-15.3-12.8z" fill="#4d841e"/></g></svg>
        </div>
        <img id="dumpster-image" src="https://i.ibb.co/DgDw640/dustbin.png">
    </div>
</div>
</template>

<script>
import { HTTP } from './../config/http-request.js'

export default {
    name: 'Login',
    data(){
        return {
        email: '',
        userPassword: '',
        message: '',
        loginApi: '/api-login.php',
        userId: null
        }
    },
    methods: {
        signInUser(){
            var that = this
            let formData = new FormData()
            formData.append('email', this.email)
            formData.append('userPassword', this.userPassword)
            HTTP.post(this.loginApi, formData)
                .then(function (response) {
                    if(response.data.status == 0){
                        that.$swal({
                        title: response.data.message,
                        icon: 'warning',
                    })     
                    }
                    if(response.data.status == 1){     
                        that.userId = response.data.data
                        that.assignId()
                        localStorage.setItem('userId', response.data.data);
                        that.$root.user = localStorage.userId
                        that.$router.push('/')
                    }
                })
                .catch(function (error) {
                    console.log(error);
                    alert(error);
            })
        },
        signUp(){
            this.$router.push('/signup')
        },
        assignId(){
            this.$root.user = this.userId
        }
    }
}
</script>

<style lang="sass" scoped>
.action-btn2, .action-btn
    margin-top: 1.5em
    width: 75%
</style>
