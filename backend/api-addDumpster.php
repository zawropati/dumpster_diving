...
$stmt = $db->prepare('UPDATE stores SET has_dumpster=1, timestamp=now() WHERE stores.id = :store_id');
  
$stmt->bindValue(':store_id', $storeId);

if(  !$stmt->execute() ){
  sendResponse(0, __LINE__, 'Couldnt update the location');
  $db->rollBack();
  exit;
}
...
sendResponse(1, __LINE__, 'Successfully added the dumpster!');
